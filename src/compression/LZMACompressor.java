...
import com.almasb.common.compression.LZ.OutWindow;
import com.almasb.common.compression.RangeCoder.BitTreeDecoder;
import com.almasb.common.compression.RangeCoder.BitTreeEncoder;
...
public class LZMACompressor {
    ...
    private static LZMAEncoder encoder = new LZMAEncoder();
    private static LZMADecoder decoder = new LZMADecoder();
     ...
     * @param data
     * @return
     *          compressed byte[] or empty byte[] if an error occurred
     */
    public static byte[] compress(byte[] data) {
        ...
        try {
            ...
            Out.e("encoder.code", "compression failed", e);
        }
    ...
    public static byte[] decompress(byte[] data) {
        ...
        try {
            ...
            byte[] properties = new byte[] {93, 0, 0, 64, 0};

            // read the size as 4 bytes
            int size = 0;
            for (int i = 0; i < 4; i++)
                size |= (in.read() << 8*i);

            // it seems by default 93, 0, 0, 64, 0
            //Out.d("properties", Arrays.toString(properties));

            decoder.SetDecoderProperties(properties);
            decoder.Code(in, out, size);
        }
        catch (IOException e) {
            Out.e("decoder.code", "decompression failed", e);
        }
    ...
    private static class Base {
        ...
        public static final int kMatchMaxLen = kMatchMinLen + kNumLenSymbols
                - 1;
    }

    // ENCODER

    private static class LZMAEncoder {
        ...
        class LiteralEncoder {
            class Encoder2 {
                ...
                public void EncodeMatched(RangeCoder.Encoder rangeEncoder,
                ...
                public int GetPrice(boolean matchMode, byte matchByte,
                        byte symbol) {
                    int price = 0;
                    int context = 1;
                    int i = 7;
                    ...
                    for (; i >= 0; i--) {
                        int bit = (symbol >> i) & 1;
                        price += RangeCoder.Encoder.GetPrice(
        ...
        class LenEncoder {
            ...
            public LenEncoder() {
                for (int posState = 0; posState < Base.kNumPosStatesEncodingMax; posState++) {
                    _lowCoder[posState] = new BitTreeEncoder(
                            Base.kNumLowLenBits);
                    _midCoder[posState] = new BitTreeEncoder(
                            Base.kNumMidLenBits);
                }
            }
            ...
            public void Encode(RangeCoder.Encoder rangeEncoder, int symbol,
            ...
            public void SetPrices(int posState, int numSymbols, int[] prices,
                    int st) {
        ...
        void Create() {
            ...
            _literalEncoder.Create(_numLiteralPosStateBits,
        ...
        int GetPureRepPrice(int repIndex, int state, int posState) {
            ...
            if (repIndex == 0) {
                ...
                if (repIndex == 1)
                    ...
                    price += RangeCoder.Encoder.GetPrice(_isRepG2[state],
                            repIndex - 2);
                }
            }
            return price;
        ...
        int GetPosLenPrice(int pos, int len, int posState) {
            ...
            return price
                    + _lenEncoder.GetPrice(len - Base.kMatchMinLen, posState);
        }
        ...
        int GetOptimum(int position) throws IOException {
            ...
            _optimum[1].Price = RangeCoder.Encoder
                    .GetPrice0(_isMatch[(_state << Base.kNumPosStatesBitsMax)
                                        ...
                                        .GetPrice(!Base.StateIsCharState(_state),
            ...
            int repMatchPrice = matchPrice
                    + RangeCoder.Encoder.GetPrice1(_isRep[_state]);
            ...
            int normalMatchPrice = matchPrice
                    + RangeCoder.Encoder.GetPrice0(_isRep[_state]);
            ...
            while (true) {
                ...
                repMatchPrice = matchPrice
                        + RangeCoder.Encoder.GetPrice1(_isRep[state]);
                ...
                for (int repIndex = 0; repIndex < Base.kNumRepDistances; repIndex++) {
                    int lenTest = _matchFinder.GetMatchLen(0 - 1,
                            reps[repIndex], numAvailableBytes);
                    if (lenTest < 2)
                        continue;
                ...
                if (newLen >= startLen) {
                    normalMatchPrice = matchPrice
                            + RangeCoder.Encoder.GetPrice0(_isRep[state]);
        ...
        public void CodeOneBlock(long[] inSize, long[] outSize,
                ...
                if (len == 1 && pos == -1) {
                    ...
                    if (pos < Base.kNumRepDistances) {
                        ...
                        if (pos == 0) {
                            ...
                            if (len == 1)
                                _rangeEncoder.Encode(_isRep0Long, complexState,
                                        0);
                            else
                                _rangeEncoder.Encode(_isRep0Long, complexState,
                                        1);
        ...
        void FillDistancesPrices() {
            ...
            for (int lenToPosState = 0; lenToPosState < Base.kNumLenToPosStates; lenToPosState++) {
                ...
                for (; i < Base.kNumFullDistances; i++)
                    _distancesPrices[st2 + i] = _posSlotPrices[st
                                                               + GetPosSlot(i)]
                                                                       + tempPrices[i];
            }
            _matchPriceCount = 0;
    ...
    private static class LZMADecoder {
        class LenDecoder {
            ...
            public int Decode(RangeCoder.Decoder rangeDecoder, int posState)
                    throws IOException {
        ...
        class LiteralDecoder {
            class Decoder2 {
                ...
                public byte DecodeWithMatchByte(
                        RangeCoder.Decoder rangeDecoder, byte matchByte)
                                throws IOException {
                    int symbol = 1;
                    do {
                        ...
                        if (matchBit != bit) {
                            while (symbol < 0x100)
                                symbol = (symbol << 1)
                                | rangeDecoder.DecodeBit(m_Decoders,
                                        symbol);
                            break;
                        }
                    } while (symbol < 0x100);
        ...
        public boolean Code(java.io.InputStream inStream,
                java.io.OutputStream outStream, long outSize)
                        throws IOException {
            ...
            while (outSize < 0 || nowPos64 < outSize) {
                ...
                if (m_RangeDecoder.DecodeBit(m_IsMatchDecoders,
                    ...
                    LiteralDecoder.Decoder2 decoder2 = m_LiteralDecoder
                    ...
                    if (m_RangeDecoder.DecodeBit(m_IsRepDecoders, state) == 1) {
                        ...
                        if (len == 0) {
                            len = m_RepLenDecoder.Decode(m_RangeDecoder,
                                    posState) + Base.kMatchMinLen;
                            state = Base.StateUpdateRep(state);
                        }
                    }
                    else {
                        ...
                        state = Base.StateUpdateMatch(state);
                        int posSlot = m_PosSlotDecoder[Base
