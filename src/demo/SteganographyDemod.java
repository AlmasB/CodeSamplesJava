...
public class SteganographyDemod {

    private static Random random = new Random();

    public static void main(String[] args) throws Exception {

        String message = "3.2 + Vector (0, 0, 1) was used in order to easily identify the coordinate system. Since it was being drawn towards the user, the Z axis points the same direction (vector's z is positive), therefore it is evident that OpenSG uses right handed coordinate system + 3.3 +"

+ "The root node is the top of the scene graph hierarchy to which we can add children in order to display them on the screen.";

        Steganography logic = new Steganography();

        /*        final BufferedImage img = new BufferedImage(1280, 720, BufferedImage.TYPE_INT_ARGB);

        Color color = new Color(10, 10, 10);

        img.setRGB(100, 100, color.getRGB());

        for (int i = 0; i < 1280; i++) {
            for (int j = 0; j < 720; j++) {
                img.setRGB(i, j, getRandomRGB());
            }
        }*/

        final BufferedImage img = ResourceManager.loadImage("ball.png");
        final BufferedImage img2 = logic.add_text(img, message);

        /*        byte[] message = "Hi, This is an encrypted message with steganography!".getBytes();
        ...
        BufferedWindow window = new BufferedWindow(1280, 720) {
            @Override
        ...
        byte[] res = logic.decode_text(logic.get_byte_data(img2));

        Out.d("result", new String(res));

        ImageIO.write(img2, "png", new File("Ball2.png"));
    }
    ...
    public static class Steganography {
        ...
        public boolean encode(String path, String original, String ext1,
            ...
            return (setImage(
                    image,
                    new File(image_path(path, stegan, "png")),
                    "png"));
        }
